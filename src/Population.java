...
public class Population {
  ...
  public Population(Pair<String, Integer>[] counts) throws IllegalArgumentException {
    ...
    /*
     * Store all the organisms in an array of pairs array's length is the total number of organisms
  ...
  public void update() {
    ...
    for (int i = 0; i < this.popArray.size(); i++) {
      ...
      /*
       * if (tempOrg.energy >= 10) { //reduce energy because organism is tired after reproducing
       * tempOrg.energy = tempOrg.energy - 10; Organism babyOrg = new Organism(); //create new
       * organism, depending on type of parent organism if (tempOrg.getType() == "Cooperator") {
       * babyOrg = new Cooperator(); } else if (tempOrg.getType() == "Defector") { babyOrg = new
       * Defector(); } else if (tempOrg.getType() == "PartialCooperator") { babyOrg = new
       * PartialCooperator(); }
       * 
       * //replace a random organism in the population with our baby Organism int randomIndex =
       * RandomizedArray(this.totalOrg)[0]; this.popArray.set(randomIndex, babyOrg); }
       */
    }
  }
  ...
  public double calculateCooperationMean() {
    /*
     * Traverse through the array, get the coop probability of each organism by calling
     * getCoopProbability() Divide this number by number of elements (saved as a field above)
     */
    double meanCoop = 0.0;
